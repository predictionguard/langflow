...
class MultiModalEmbedderComponent(Component):
    display_name: str = "Multi Modal Embedder"
    description: str = "Generate embeddings for a given image-text pairs using the specified embedding model."
    ...
    def generate_embeddings(self) -> Data:
        try:
            ...
            embeddings = embedding_model.embed_image_text([{"text": text_content, "image": image}])
            if not embeddings or not isinstance(embeddings, list):
                msg = "Invalid embeddings generated"
                raise ValueError(msg)

            embedding_vector = embeddings[0]
            self.status = {"text": text_content, "image": image, "embeddings": embedding_vector}
            return Data(data={"text": text_content, "image": image, "embeddings": embedding_vector})
        except Exception as e:
            logging.exception("Error generating embeddings")
            error_data = Data(data={"text": "", "image": "", "embeddings": [], "error": str(e)})
